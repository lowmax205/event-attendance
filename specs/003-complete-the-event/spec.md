...
- **What happens when multiple admins edit the same user account simultaneously?** System should use optimistic locking or display a warning if changes conflict, showing the most recent update.
...
- **FR-013**: Event Management interface MUST display all events with columns: event name, date/time, location, created by, status (Upcoming/Active/Completed), and total attendances
...
- **FR-046**: System MUST display a loading indicator during export generation for large datasets
- **FR-047**: System MUST handle export errors gracefully and allow retry
- **FR-048**: System MUST log export actions to audit trail with user ID, timestamp, and filter criteria
...
- **AnalyticsSummary**: Aggregated metrics for dashboard display. Contains time period reference, total events count, total attendances count, average attendance per event, verification rate percentage, pending verification count, attendance rates by department (department name and percentage), attendance rates by course (course name and percentage), and metric calculation timestamp.
...
- **AuditLog**: Extended from Phase 1. Now includes additional event types: user_role_changed, user_status_changed, event_created, event_edited, attendance_verified, attendance_rejected, dispute_resolved, data_exported.
